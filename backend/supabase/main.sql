...
create table
  public.t_crypto_assets (
    ...
    constraint t_crypto_assets_pkey primary key (id),
    constraint t_crypto_assets_symbol_key unique (symbol)
  ) tablespace pg_default;

create table
  public.t_prediction_periods (
    id bigint generated by default as identity not null,
    asset_id bigint null,
    starts_at timestamp with time zone not null,
    ends_at timestamp with time zone not null,
    is_active boolean null default true,
    buy_in double precision not null default 1.0,
    total_pool double precision null default 0,
    current_price numeric(20, 8) null,
    current_high numeric(20, 8) null,
    current_low numeric(20, 8) null,
    final_high numeric(20, 8) null,
    final_low numeric(20, 8) null,
    closed_at timestamp with time zone null,
    created_at timestamp with time zone null default now(),
    updated_at timestamp with time zone null default now(),
    constraint t_prediction_periods_pkey primary key (id),
    constraint t_prediction_periods_asset_id_fkey foreign key (asset_id) references t_crypto_assets (id)
  ) tablespace pg_default;

create table
  public.t_prize_distributions (
    id bigint generated by default as identity not null,
    period_id bigint null,
    winner_wallet_address text not null,
    prize_amount double precision not null,
    ton_transaction_hash text null,
    status text not null default 'pending'::text,
    created_at timestamp with time zone null default now(),
    completed_at timestamp with time zone null,
    user_prediction_id bigint null,
    constraint t_prize_distributions_pkey primary key (id),
    constraint t_prize_distributions_ton_transaction_hash_key unique (ton_transaction_hash),
    constraint t_prize_distributions_period_id_fkey foreign key (period_id) references t_prediction_periods (id),
    constraint t_prize_distributions_user_prediction_id_fkey foreign key (user_prediction_id) references t_user_predictions (id)
  ) tablespace pg_default;

create table
  public.t_user_predictions (
    id bigint generated by default as identity not null,
    wallet_address text not null,
    period_id bigint null,
    predicted_high numeric(20, 8) not null,
    predicted_low numeric(20, 8) not null,
    ton_transaction_hash text not null,
    high_difference numeric(20, 8) null,
    low_difference numeric(20, 8) null,
    total_difference numeric(20, 8) null,
    created_at timestamp with time zone null default now(),
    constraint t_user_predictions_pkey primary key (id),
    constraint t_user_predictions_ton_transaction_hash_key unique (ton_transaction_hash),
    constraint t_user_predictions_wallet_period_unique unique (wallet_address, period_id),
    constraint t_user_predictions_period_id_fkey foreign key (period_id) references t_prediction_periods (id)
  ) tablespace pg_default;
...
declare
    v_current_period t_prediction_periods;
    v_next_period t_prediction_periods;
...
begin
    ...
    if v_current_period is null then
        raise exception 'No active prediction period found';
    end if;
    ...
    into v_next_period
    from t_prediction_periods
    where id = v_current_period.id + 1
    and is_active = true;
    
    if v_next_period is null then
        raise exception 'Next prediction period not found or not active';
    end if;
    ...
    insert into t_user_predictions (
        ...
        p_wallet_address,
        v_next_period.id,  -- Using next period's ID instead of parameter
        p_predicted_high,
        p_predicted_low,
...
returns table (
    ...
    winner_prize_amount double precision,
    winner_transaction_hash text,
    winner_distribution_status text,
    ...
    select 
        ...
        pd.prize_amount as winner_prize_amount,
        pd.ton_transaction_hash as winner_transaction_hash,
        pd.status as winner_distribution_status,
