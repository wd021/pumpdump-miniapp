// dutch

export const nl = {
  home: {
    "Today's prize pool": "De prijs van vandaag",
    "Current BTC price": "De huidige BTC-prijs",
    "Today's High": "De hoogste van vandaag",
    "Today's Low": "De laagste van vandaag",
    "Leading Prediction": "Vooraanstaande voorspelling",
    "Your Prediction": "Uw voorspelling",
    "No predictions for this round": "Geen voorspellingen voor deze ronde",
    "Enter Tomorrow's Race": "Doe mee aan de race van morgen",
    "Connect Wallet to Predict": "Verbind je portemonnee om te voorspellen",
    "Join the prediction game by connecting your wallet":
      "Doe mee aan het voorspellingsspel door je portemonnee te verbinden",
    History: "Geschiedenis",
    About: "Over",
    "You haven't made a prediction for this round":
      "U heeft geen voorspelling gedaan voor deze ronde",
    "Loading...": "Laden...",
    "Loading price data...": "Prijsgegevens worden geladen...",
    "Error:": "Fout:",
    "Your Prediction for Tomorrow": "Uw voorspelling voor morgen",
    "Come back tomorrow to submit your prediction for the following day!":
      "Kom morgen terug om uw voorspelling voor de volgende dag in te dienen!",
  },
  predict: {
    "Predict Tomorrow's BTC Price": "Voorspeld de BTC-prijs van morgen",
    "Predict High": "Voorspeld de hoogste",
    "Enter predicted high...": "Voer de voorspelde hoogste in...",
    "Predict Low": "Voorspeld de laagste",
    "Enter predicted low...": "Voer de voorspelde laagste in...",
    "Most accurate prediction wins!": "De meest accurate voorspelling wint!",
    "Get closest to tomorrow's actual high & low to win the entire prize pool":
      "Kom zo dicht mogelijk bij de hoogste en laagste waarden van morgen om de hele prijs te winnen",
    "Submit Prediction": "Verzenden voorspelling",
    "Submitting...": "Verzenden...",
    "Processing your prediction...": "Uw voorspelling wordt verwerkt...",
    "Success!": "Succes!",
    "Your prediction has been submitted successfully! Good luck! 🍀":
      "Uw voorspelling is succesvol verzonden! Succes! 🍀",
    Error: "Fout",
    "An unexpected error occurred": "Er is een onverwachte fout opgetreden",
    "Try Again": "Probeer het opnieuw",
  },
  history: {
    "Past Predictions": "Voorgaande voorspellingen",
    "Loading history...": "Geschiedenis wordt geladen...",
    "No prediction history found": "Geen voorspellinggeschiedenis gevonden",
    Settled: "Afgerekend",
    Pending: "In afwachting",
    "Final Price": "Eindprijs",
    "Your Prediction": "Uw voorspelling",
    Winner: "Winnaar",
    "To be settled": "Af te rekenen",
    "Loading predictions...": "Voorspellingen worden geladen...",
    "All Predictions": "Alle voorspellingen",
  },
  about: {
    "How It Works": "Hoe het werkt",
    "Ready to make your prediction?": "Klaar om uw voorspelling te doen?",
    "Predict BTC Prices": "Voorspeld de BTC-prijzen",
    "Set tomorrow's BTC high & low prices":
      "Stel de hoogste en laagste BTC-prijzen van morgen in",
    "Pay 1 TON": "Betal 1 TON",
    "Submit your prediction to join":
      "Verzenden uw voorspelling om deel te nemen",
    "Win Prize Pool": "Win de prijs",
    "Most accurate prediction wins the entire prize pool. Pump Dump takes a 5% fee.":
      "De meest accurate voorspelling wint de hele prijs. Pump Dump neemt een vergoeding van 5%.",
  },
} as const;
